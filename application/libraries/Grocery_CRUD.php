...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$input .= $is_image ? " $image_class'><img src='".$file_url."' height='150px'>" : "' target='_blank'>$value";
		$input .= "</a> ";
...
//		$this->field_types = null;
		...
		foreach ($read_fields as $field) {
			...
			if (!empty($this->field_types[$field->field_name]->crud_type)
					&&  $this->field_types[$field->field_name]->crud_type == 'upload_file') {
							
				$this->field_types[$field->field_name]->crud_type = 'upload_file';
				continue;
			}
			$this->field_types[$field->field_name]->crud_type = 'readonly';			
			$this->field_types[$field->field_name]->type = 'varchar';			
		}
